...
@Autonomous(name = "AutoYellow5", group = "Examples")
public class AutoYellow5 extends basic {
    ...
    private final Pose pickup3Pose = new Pose(29, 130, Math.toRadians(25));
    ...
    private final Pose pickup5Control = new Pose(65, 128, Math.toRadians(270));
    private final Pose pickup5Pose = new Pose(65, 95, Math.toRadians(270));
    ...
    public void autonomousPathUpdate() {
        ...
        // 5
            ...
            armTarget = arm_catch;
            if (pathTimer.getElapsedTimeSeconds() >= 0.4) setPathState(8);
        } else if (pathState == 8) {
            wristCombo(wrist_bucket, 0);
            clawCombo(claw_Close, intake_off);
            if (!follower.isBusy()) {
            ...
            clawCombo(claw_Open, -intake_on);
            if (pathTimer.getElapsedTimeSeconds() >= putBucketTime) setPathState(55);
        }

        // End

        else if (pathState == 55) {
            slideTarget = smin;
        ...
        } else if (pathState == 56) {
            if (pathTimer.getElapsedTimeSeconds() > 0.5) setPathState(57);
        }else if (pathState == 57) {
            armTarget = 52;
            setPathState(58);
        } else if (pathState == 58) {
            if (!follower.isBusy()) {
                setPathState(-1);
            }
        }
